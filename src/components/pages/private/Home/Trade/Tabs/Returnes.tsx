...
const Returnes: React.FC<Props> = ({navigation, route}) => {
  ...
  const refreshing = useAppSelector(
    state => state.trade.tabs.return.refreshing,
  );
  ...
  const handleRefresh = () => loadReturns(true, false, 0);
    ...
    return (
      <Layout style={styles.headerWrap}>
        ...
        <View style={[styles.item, styles.margin]}>
          ...
          <Text status="primary">{addSpaces(head.sumReturn)}</Text>
        </View>
      </Layout>
    );
  }, [head]);
      ...
      <List
        ...
        renderItem={props => (
          <ListItem
            ...
            reload={() => loadReturns(false, false, 0)}
          />
        )}
      />
